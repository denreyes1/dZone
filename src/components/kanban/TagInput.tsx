...
export const TagInput = forwardRef<TagInputHandle, TagInputProps>(
  function TagInput({ tags, onChange }, ref) {
    ...
    function addTagWith(label: string, color: string): TaskTag[] {
      ...
      if (tags.some((t) => t.label.toLowerCase() === trimmed.toLowerCase())) return tags;
    ...
    function addTag() {
      addTagWith(input, selectedColor);
    }

    function addSuggestion(suggestion: TaskTag) {
      addTagWith(suggestion.label, suggestion.color);
    ...
    useImperativeHandle(ref, () => ({
      flush(): TaskTag[] {
        return addTagWith(input, selectedColor);
      },
    }));
    ...
    const suggestions = history
      .filter(
        (h) =>
          !tags.some((t) => t.label.toLowerCase() === h.label.toLowerCase()) &&
          (input === '' || h.label.toLowerCase().includes(input.toLowerCase())),
      )
          ...
          <div className="flex flex-wrap gap-1">
            {suggestions.map((s, i) => (
              <button
                ...
                className="rounded-full px-2 py-0.5 text-[11px] font-medium transition-all hover:brightness-125"
                style={{
                  backgroundColor: s.color + '15',
                  color: s.color,
                  border: `1px dashed ${s.color}30`,
                }}
              >
                + {s.label}
              </button>
